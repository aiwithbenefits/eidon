...
SIMILARITY_THRESHOLD = 0.85  # For MSSIM, range 0-1. Higher means more similar.
MIN_HAMMING_DISTANCE = 5     # For perceptual hash (phash). Lower means more similar.
...
HOT_DAYS = 7          # Files newer than this remain in the primary 'screenshots' dir.
COLD_DAYS = 30        # Files older than this are candidates for moving to the archive.
...
if __name__ == "__main__" or "pytest" not in sys.modules: # Crudely check if not run by pytest
    ...
    class ArgsDefault:
        def __init__(self):
            ...
            self.primary_monitor_only = False
    args = ArgsDefault()
